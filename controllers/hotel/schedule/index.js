...
// exports.addSpaSchedule = ({hotel_id, date, therepist}) => {
//   return new Promise(async (resolve, reject) =>{
//     if(!therepist||!hotel_id||!date) reject('hotel_id||date||therepist params are missing');
//     date = new Date(date);
//     date.setTime(date.getTime() + date.getTimezoneOffset() * 60 * 1000  + (/* UTC+8 */ 2) * 60 * 60 * 1000);
//     date.setDate(date.getDate()+1);
//     console.log(date)
//     await Schedule.findOne({hotel:hotel_id,date}, (err,check)=>{
//       if(err) return reject(err.message);
//       else if(!check) return reject('hotel_id || date not exist');
//     });
//     await Schedule.findOne({hotel:hotel_id, date, spa:{$not:{$elemMatch:{therepist}}}}, async (err, schedule) =>{
...
// exports.getScheduleByHotel = ({hotel_id}) => {
//   return new Promise(async (resolve, reject) => {
//     if(!hotel_id) reject('hotel_id param is missing');
//     Schedule.find({hotel:hotel_id}, (err, schedule) => {
//       if(err) return reject(err.message);
//       else if(!schedule) return reject("no schedule for this hotel_id");
//       return resolve(schedule);
//     }).sort({date:1});
//   })
// }
...
// exports.getSpaAvailable = ({hotel_id})=>{
//   return new Promise(async (resolve,reject)=>{
//     if(!hotel_id)return reject('hotel_id param is missing');
//     let now = new Date();
//     await Schedule.find({hotel: hotel_id, date:{ $gte: now }}, async (err,schedule)=>{
//       if(err) return reject(err.message);
...
// exports.getSpaAvailableByDate = ({hotel_id , date})=>{
//   return new Promise(async (resolve,reject)=>{
//     console.log(1)
//     if(!hotel_id ||!date)return reject('hotel_id  || date params are missing');
//     date = new Date(date);
//     date.setTime(date.getTime() + date.getTimezoneOffset() * 60 * 1000  + (/* UTC+8 */ 2) * 60 * 60 * 1000);
//     date.setDate(date.getDate()+1);

//     await Schedule.findOne({hotel: hotel_id, date}, async (err,schedule)=>{
...
//       if(err) return reject(err.message);
//       console.log(schedule)
//       if(!schedule) return reject("no such schedule exist");

//       let appointment = _.filter(schedule.spa, spa => spa.id == appointment_id)
//       //save new appointment in user
//       user.spa = user.spa.concat({appointment: appointment[0].id});
//       await user.save();
